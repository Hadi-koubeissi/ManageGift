...
exports.run = async (client, message, args, guildData, lang) => {
  ...
  if (!giveawayChannel)
    return message.channel.send(lang.create.channel).then((msg) => {
      msg.delete({ timeout: 10000 });
      message.delete({ timeout: 10000 });
    });

  if (message.member.guild.id != giveawayChannel.guild.id)
    return message.channel.send(lang.create.otherServer);
  ...
  if (
    !giveawayDuration ||
    isNaN(ms(giveawayDuration)) ||
    ms(giveawayDuration) < ms("1m")
  )
    return message.channel.send(lang.create.duration).then((msg) => {
      msg.delete({ timeout: 10000 });
      message.delete({ timeout: 10000 });
    });
  ...
  if (isNaN(giveawayNumberWinners))
    return message.channel.send(lang.create.argswinners).then((msg) => {
      msg.delete({ timeout: 10000 });
      message.delete({ timeout: 10000 });
    });
  ...
  if (!giveawayPrize)
    return message.channel.send(lang.create.prize).then((msg) => {
      msg.delete({ timeout: 10000 });
      message.delete({ timeout: 10000 });
    });
