...
exports.run = async (client, message, args, guildData, lang) => {
  ...
  if (
    ...
    return message.channel.send(lang.start.duration).then((msg) => {
  ...
  if (isNaN(giveawayNumberWinners)) {
    return message.channel.send(lang.start.argswinners).then((msg) => {
      msg.delete({ timeout: 10000 });
      message.delete({ timeout: 10000 });
    });
  }
  ...
  if (!giveawayPrize) {
    return message.channel.send(lang.start.prize).then((msg) => {
      msg.delete({ timeout: 10000 });
      message.delete({ timeout: 10000 });
    });
  }
