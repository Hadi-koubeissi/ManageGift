...
module.exports = {
  ...
  edit: {
    ...
    timepr: (numberOfSecondsMax) =>
      `${e.success} The giveaway prize will updated in less than \`${numberOfSecondsMax}\` seconds.`,
    timwi: (numberOfSecondsMax) =>
      `${e.success} The giveaway winner count will updated in less than \`${numberOfSecondsMax}\` seconds.`,
    err: `${e.error} Unable to recognize this id giveaway: `,
  ...
  myprefix: {
    hello: `${e.info} |  Hello `,
    prefixis: (guildData) =>
      ` my prefix on this server is \`${guildData.prefix}\`.Use \`${guildData.prefix}help\` to get the list of the commands!`,
  },
  blacklist: {
    blacklist:
      ":lock: **You cannot use `ManageGift commands` because you are in the `Blacklist!!`**",
  },
};
