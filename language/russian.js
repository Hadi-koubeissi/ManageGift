...
module.exports = {
	...
	set: {
		perms: `${e.error} У Вас нет права \`MANAGE_MESSAGES\`, Вы можете создать роль для использования данной команды:`,
		msg: `${e.error} Укажите раздел для изменения настроек! Параметр \`mention\` будет оповещать пользователей когда розыгрыш начался, \`role\` для предоставления доступа к командам розыграша.`,
