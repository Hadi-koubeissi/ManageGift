...
module.exports = {
  ...
  create: {
    ...
    winMessage: (message) =>
      "🎉 | Поздравляем, {winners}! Ты выиграл: **{prize}**!",
  ...
  edit: {
    ...
    timepr: (numberOfSecondsMax) =>
      `${e.success} Приз розыгрыша будет обновлён через \`${numberOfSecondsMax}\` секунд.`,
    timwi: (numberOfSecondsMax) =>
      `${e.success} Кол-во победителей будет обновлено через \`${numberOfSecondsMax}\` секунд.`,
  ...
  myprefix: {
    hello: `${e.info} | Привет `,
    prefixis: (guildData) =>
      ` мой префис на данном сервере: \`${guildData.prefix}\`. Используйте команду \`${guildData.prefix}help\`, чтобы получить список всех команд!`,
  },

  blacklist: {
    blacklist:
      ":lock: **Вы не можете использовать `ManageGift команды`, т.к Ваш сервер был добавлён в `чёрный список`!**",
  },
};
